...
\item The result: \code{FAILED}, \code{PASSED} or \code{SIDEFX}. The latter only occurs
  ...
  \begin{itemize}
    \item \code{[envv]} An environmental variable has changed.
   \end{itemize}
...
include setting, removing, or changing global variables, environment variables,
or global options. Side effects may have unwanted consequences for expressions
or function calls after they have been set and can lead to bugs that are hard
to trace. 

If you add \code{report\_side\_effects()} anywhere in a test file, side effects
are monitored from that point on, for that file only. It can be switched off by
setting \code{report\_side\_effects(FALSE)}.  The \code{report\_side\_effects}
functionality will compare the state of environment variables before and after
every expression in the test file and report any changes. See
\code{?report\_side\_effects} for the type of side effects that are currently
supported.

\pkg{tinytest} will still unset any options and environment
variables after running a test file that are set explicity with \code{options}
or \code{Sys.setenv} in a test file (See \S\ref{sect:running}).
...
(\code{expect}-funcitons).  To use such a package:
\begin{enumerate}
\item Add the extension package to the \code{Suggests:} field in the \code{DESCRIPTION} file.
\item Add \code{library(pkg)} or \code{require(pkg)} to the test files that use the extensions.
\end{enumerate}
...
functions using namespace resolution as in \code{pkg::expect\_something}, because
in that case the test result will not be caught by \pkg{tinytest}.

See \code{?register\_tinytest\_extension} for a complete description.
...
sitiations, all bets on file order are off.


\subsubsection*{\code{test\_package}.} 
This function assumes that a package is installed. It can gather any
information necessary to paralellize a test run. The simplest way to
paralellize is to specify the number of CPUs used.
<<eval=FALSE>>=
...
reinstalled, it should also be reloded.  It is in general hard to completely
