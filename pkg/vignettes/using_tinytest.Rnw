...
@
A function that is called by the user should only use data that is  passed as
an argument, and it should also not alter anything outside of its scope. A
function that satisfies these requirements is called a \emph{pure function}.
You can \emph{in principle} define pure functions by tabulating inputs and
outputs.
<<>>=
...
perform unit tests. Pure functions are ideally suited to be subjected to unit
testing.
