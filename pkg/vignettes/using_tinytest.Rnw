...
It is a good practice to create test files that run independent
of other test files. For this reason, \pkg{tinytest} records
\begin{itemize}
\item options, set with \code{options()};
\item environment variables, set with \code{Sys.setenv()}
\end{itemize}
After a test file is executed, all options and environment variables set during
the test run are reset to their original values (i.e.  the values prior to
calling \code{run\_test\_file}). The default behavior can be toggled as follows.
<<eval=FALSE>>=
