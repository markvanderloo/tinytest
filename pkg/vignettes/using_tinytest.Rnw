...
I expect that readers of this document know and understand how to write R
functions.  If you want to use \pkg{tidytest} for your package, I expect that
you have a basic understanding of the directory structure that consitutes the
source of an R package.
...
@
A function that is called by the user should not use data that is not passed as
an argument, and it should also not alter anything outside of it's scope. A
function that satisfies these requirements is called a \emph{pure function}.
You can \emph{in principle} define them by tabulating inputs and outputs.
<<>>=
...
@
Pure functions are very suited to be subjected to unit testing.
...
@
We like to check a few things before we trust it. For example, we expect
that 
...
When I work on a function, I usually first write a small script. When that
basically works, I stick it in a function, and then I open a second file with
some small tests calling the function to try and test it. I usually have a file
open that calls the function and a file with the function itself, while I am
editing and improving it. The idea of the \pkg{tinytest} package is that you
can very easily integrate those little test scripts into a package (or just
keep using them as part of a set of production scripts).
