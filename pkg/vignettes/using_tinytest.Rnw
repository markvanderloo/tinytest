...
The default package structure is very simple. 
\begin{enumerate}
...
\begin{verbatim}
    test_package("NAMEOFYOURPACKAGE")
\end{verbatim}
\end{enumerate}

In a terminal, you can now do
\begin{verbatim}
    R CMD build YOURPACKAGEDIRECTORY
...
detect whether a package is tested at one of the CRAN's machines. However, you
can \emph{force} a test at your own environment, which will then be skipped
everywhere else. Since skipping tests should be an exception rather than a
rule, forcing tests locally also makes more sense.
