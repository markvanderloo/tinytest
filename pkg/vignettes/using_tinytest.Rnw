...
It is important that package developers form their own opinion about workflow
and development cycle. Not only because experience levels and taste may vary,
but because diversity is important for driving innovation.



Therefore, this package tries to be modest in its opinions. The most important
opinion is the following.
\begin{itemize}
\item It is a good idea to install tests with a package. 
\end{itemize}
This way, users who installed the package can run tests and return their
results to the package author.


Other than that, the package tries to be a set of tools that can be used
flexibly.
...
The result of an \code{expect\_} function is a \code{tinytest} object. You can print them
in long format (default) or in short, one-line format like so.
<<>>=
...
tries to be smart about printing them, but the user has ultimate control over
this behaviour. 

\begin{itemize}
\item\code{options(tt.format="auto")} 
\end{itemize}
...
@
This will build the package, install it in a temporary directory and run all te
tests. 
...
and then run all the test. The function
<<eval=FALSE>>=
  build_install_test()
@
...
When you call a function, you can think of it's arguments flowing through a
